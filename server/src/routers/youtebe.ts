...
router.get(
  ...
  async (req: Request, res: Response<any>) => {
    const { q } = req.query;
    const data = await getVideoListByYtsearch({
      q: q as string,
    });
    res.json({
      data,
...
router.post(
  ...
  async (
    req: Request,
    res: Response<UserDetailResponse | ErrorResponse>
  ) => {
    try {
      const { q, type, count }: UserDetailRequest =
        req.body;
      ...
      res
        .status(500)
        .json({ error: '启动任务失败：' + error.message });
    }
  }
);
...
router.delete(
  ...
  async (
    ...
    try {
      ...
      res
        .status(500)
        .json({ error: '停止任务失败：' + error.message });
    }
  }
);
